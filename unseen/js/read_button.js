...
function addButtonToChatWindow(chtWindow, conversationUrl) {
  ...
  if ( chtWindowFooter.find('.mark_as_read').length > 0) {  return } // Button is already added. Should not continue
  $.ajax({
    type: 'GET',
    url: conversationUrl
  }).done(function (response) {
    if ( chtWindowFooter.find('.mark_as_read').length > 0) {  return }
    chtWindowFooter.prepend('<div class="mark_as_read">Mark as read</div>');
    var conversationData = getConversationDataObject(response);
    var threads = conversationData.jsmods.instances[1][2][1].threads;
    var allParticipants = conversationData.jsmods.instances[1][2][1].participants;
    var partnerId = getPartnerId(conversationUrl, allParticipants);
    var readUrl = getReadUrl(partnerId, threads);
